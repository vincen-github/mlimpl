...
class DecisionClassifier(object):
    ...
    def __createTree(self, X, y, candidate_features):
        """
        __createTree():
        ...
        parameters:
            ...
            2. y:The label matrix of smaples which in current leaf node.
                dtype : Series
            candidate_features:In every epoch of recursively.We need to select the best split feature from candidate_features
                dtype:list
        """
        """
        ...
        if candidate_features == []:
            return y[np.argmax(y.value_counts())]
        # 否则,选择纯度增益最大的的特征,
        best_split_feature, biggest_gain = self.__select_best_split_feature(X, y, candidate_features)
        # 将candidate_features中的best_feature_value删除
        candidate_features.remove(best_split_feature)
        ...
        for best_feature_value in self.__all_feature_dict[best_split_feature]:
            ...
            tree[best_split_feature][best_feature_value] = self.__createTree(X = X.loc[index], y = y[index], candidate_features = candidate_features)
        return tree
